...
export interface ProgressStorage {
  readLedger(): Promise<Ledger | null>
  writeLedger(value: Ledger): Promise<void>
  readToken(): Promise<StravaToken | null>
  writeToken(token: StravaToken): Promise<void>
  readProgress(): Promise<PublicProgress | null>
  writeProgress(value: PublicProgress): Promise<void>
}
...
function shouldRetryStravaResponse(response: Response): boolean {
...
function fixedNumber(value: unknown, digits: number): string {
  const numberValue = Number(value)

  return Number.isFinite(numberValue) ? numberValue.toFixed(digits) : ''
}

async function getAccessToken(storage: ProgressStorage, env: NodeJS.ProcessEnv): Promise<string> {
  ...
  const token = (await response.json()) as StravaToken
  await storage.writeToken(token)
  return token.access_token
}

async function fetchClubActivities(accessToken: string, clubId: string): Promise<unknown[]> {
...
function normalizeActivity(activity: any): NormalizedActivity | null {
...
export async function runUpdate({
  ...
  const progressState: Ledger = (await storage.readLedger()) ?? {
    clubId,
  ...
  const rideActivities = fetchedActivities
  ...
  const publicProgress: PublicProgress = {
    clubId,
